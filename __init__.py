...
class Elf:
    ...
    def __init__(self, initstr):
        ...
        (If you only give it 64 bytes, it'll just parse the header, if you give it
        ...
        if len(initstr) > 0:
            if os.path.exists(initstr[:initstr.find('\0')]):
                bytes = file(initstr, "rb").read()
                self.myname = initstr

            self.initFromBytes(bytes)

            # If we only got the 64 bytes, we have
...
class Elf32Reloc:
    ...
    def initFromBytes(self,bytes):
        (self.r_offset, self.r_info) = struct.unpack("2I",bytes[:len(self)])
...
class Elf32Reloca(Elf32Reloc):
    ...
    def initFromBytes(self, bytes):
        (self.r_offset, self.r_info, self.r_addend) = struct.unpack("2Ii", bytes[:len(self)])

    def __len__(self):
        return struct.calcsize("2Ii")
...
class Elf64Reloca(Elf32Reloca):
    def initFromBytes(self, bytes):
        (self.r_offset, self.r_info, self.r_addend) = struct.unpack("2Qq", bytes[:len(self)])

    def __len__(self):
        return struct.calcsize("2Qq")
...
class Elf32Pheader:
    ...
    def __repr__(self):
        return "[%35s] VMA: 0x%.8x  offset: %8d  memsize: %8d  align: %8x  (filesz: %8d)  flags: %x" % (
            self.getTypeName(),
            self.p_vaddr,
            self.p_offset,
            self.p_memsz,
